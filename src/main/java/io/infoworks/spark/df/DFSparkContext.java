...
public class DFSparkContext {
  ...
  public void runBasicDataSourceExample() {
    // $example on:generic_load_save_functions$
    Dataset<Row> usersDF = spark.read().format("parquet").load(INPUTPATH + "/users.parquet");
    usersDF.select("name", "favorite_color").write().format("json").save( OUTPUTPATH+"/namesAndFavColors.json");
    // $example off:generic_load_save_functions$
    // $example on:manual_load_options$
    Dataset<Row> peopleDF =
      spark.read().format("json").load(INPUTPATH +"/people.json");
    peopleDF.select("name", "age").write().format("json").save(OUTPUTPATH+"/namesAndAges.json");
    // $example off:manual_load_options$
    // $example on:direct_sql$
    Dataset<Row> sqlDF =
      spark.sql("SELECT * FROM parquet.`" + INPUTPATH + "/users.parquet`");
  ...
  public void runJsonDatasetExample() {
    // $example on:json_dataset$
    // A JSON dataset is pointed to by path.
    // The path can be either a single text file or a directory storing text files
    Dataset<Row> people = spark.read().json(INPUTPATH + "/people.json");
  ...
  public Dataset<Row> getDataFrame(Format format) {
    return null;
  }
